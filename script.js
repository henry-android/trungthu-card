...
// Tạo hạt sáng nhè nhẹ quanh card để tăng cảm giác lãng mạn mà không làm rối
function createSparkle(container) {
  if (!container) return;
  const sparkle = document.createElement("span");
  sparkle.className = "sparkle";
  sparkle.style.left = Math.random() * 82 + 9 + "%";
  sparkle.style.top = Math.random() * 68 + 15 + "%";
  sparkle.style.animationDelay = Math.random() * 1.2 + "s";
  container.appendChild(sparkle);
  setTimeout(() => sparkle.remove(), 4300);
}
setInterval(() => createSparkle(appContent), 420);
// Chạy đom đóm ngoài màn hình nền
if (desktopBg) setInterval(() => createFirefly(desktopBg), 400);

// 3. HIỆU ỨNG GÕ CHỮ (TYPEWRITER)
const message =
  "Dưới bầu trời rực rỡ muôn vàn vì sao và ánh trăng thanh bình, anh chỉ muốn gửi đến em những điều ngọt ngào nhất. Cảm ơn em đã là ánh sáng rực rỡ nhất trong cuộc đời anh. Chúc em một mùa Trung Thu thật ấm áp, viên mãn và tràn ngập tiếng cười. Trái tim anh luôn hướng về em. Yêu em vô vàn! 🌕🏮❤️";
